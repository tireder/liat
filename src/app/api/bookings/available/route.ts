...
export async function GET(request: NextRequest) {
    try {
        ...
        // Get blocked slots (global + artist-specific)
        ...
        if (artistId) {
            blockedQuery = blockedQuery.or(`artist_id.is.null,artist_id.eq.${artistId}`);
        }
        ...
        // Get existing bookings (filter by artist if provided)
        ...
        if (artistId) {
            bookingsQuery = bookingsQuery.eq("artist_id", artistId);
        }
