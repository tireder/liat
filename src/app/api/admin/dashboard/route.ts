...
export async function GET() {
    try {
        ...
        const weekStats = { total: monthTotalCount || 0, confirmed: 0, completed: 0, cancelled: 0, pending: 0 };
        weekBookings?.forEach((b: { status: string }) => {
            // Count all for breakdown
            if (b.status === "confirmed") {
                weekStats.confirmed++;
            }
            if (b.status === "completed") weekStats.completed++;
            if (b.status === "cancelled") weekStats.cancelled++;
            if (b.status === "pending" || b.status === "pending_change") {
                weekStats.pending++;
            }
        });
